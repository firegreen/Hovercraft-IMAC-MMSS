...
void drawView(const Level* l, int currentHovercraft){
    ...
    for(i=0;i<l->nbBots;i++){
        ...
        glBegin(GL_TRIANGLES);
            glVertex2f(l->bots[i].h.physical_body.x+cosf(l->bots->h.physical_body.angle*M_PI/180.+1)*4,
                       l->bots[i].h.physical_body.y);
            glVertex2f(l->bots[i].h.physical_body.x+ l->bots[i].path->direction.x*20,
                       l->bots[i].h.physical_body.y+ l->bots[i].path->direction.y*20);
            glVertex2f(l->bots[i].h.physical_body.x-cosf(l->bots->h.physical_body.angle*M_PI/180.-1)*4,
                       l->bots[i].h.physical_body.y);
        glEnd();*/
...
void initLevel(Level *level, int mapid, int nbplayers, int nbBots){
    ...
    if(error!=NOERROR){
        fprintf(stderr,"Error while reading map %d, ",mapid);
        switch(error){
